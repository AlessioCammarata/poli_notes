...
// File di input con gli elementi disponibili
...
// Struttura che rappresenta un elemento del catalogo
...
// Rappresentazione di una diagonale generata
...
int leggiFile(FILE *fin, Elements **e, int n){
    ...
    for(i = 0; i<n; i++){
        ...
        if(fscanf(fin,"%s %d %d %d %d %d %f %d",
                  ...
                  &e[i]->value,
                  &e[i]->difficulty) != 8 ){
            free(e[i]); free_all(e, i); return 0;
        }
    }
    return i==n;
}
...
static void gen_diagonals_rec(Elements **e, int n, int DD,
                              ...
                              int has_forward, int has_backward,
                              DiagList *out){
    // Opzione: registrare la diagonale corrente (se non vuota e valida)
    ...
    for (int i=0;i<n;i++){
        ...
        if (new_diff > DD) continue; // bound sulla difficoltà di diagonale
